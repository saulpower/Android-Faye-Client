...
public class SlidingDrawerRightSide extends ViewGroup {
    public static final int ORIENTATION_HORIZONTAL = 0;
     ...
     /**
      * Creates a new DragContentSlidingDrawer from a specified set of attributes defined in XML.
      *
    ...
    protected void dispatchDraw(Canvas canvas) {
     ...
     public boolean onInterceptTouchEvent(MotionEvent event) {
         ...
         if (action == MotionEvent.ACTION_DOWN) {
             ...
             if (frame.contains((int) x, (int) y)) {
                 // Touch in handle. Start tracking drag immediately.
                 startTrackingDrag (event);
                 return true;
             } else {
                 // Touch in content, not in handle. Save the start point in case this turns into a drag.
                 mTouchStarted = true;
                 mTouchStartX = x;
                 mTouchStartY = y;
             }
             ...
             if (mTouchStarted) {
                 // Check if we've moved enough from the start position to consider this a drag event.
                 if (mVertical) {
                     if (Math.abs(mTouchStartY - y) > mTouchSlop) {
                         startTrackingDrag (event);
                         return true;
                     }
                 } else {
                     if (Math.abs(mTouchStartX - x) > mTouchSlop) {
                         startTrackingDrag (event);
                         return true;
                     }
                 }
     ...
     private void startTrackingDrag (final MotionEvent event) {
         float x = event.getX();
         float y = event.getY();
         final View handle = mHandle;
