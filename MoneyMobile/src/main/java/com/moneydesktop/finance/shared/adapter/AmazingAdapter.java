...
public abstract class AmazingAdapter extends BaseAdapter implements SectionIndexer, OnScrollListener {
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        mScrolling = (scrollState != 0);

        if (!mScrolling && mPending) {
            mPending = false;
            onNextPageRequest(mPageRequested);
        }
    }
    ...
    private void onNextPageRequest(int page) {

        if (!mScrolling) {
            onNextPageRequested(page);
        } else {
            mPending = true;
            mPageRequested = page;
        }
